...
function Map({ children, styles, ...props }: MapProps) {
  ...
  useEffect(() => {
    if (mapRef.current) {
      ...
      // Basic check: if currentStyle is undefined or doesn't match newStyle URI
      // Note: getStyle() returns the full JSON, so we might need a better comparison or track current URI
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "rounded-md bg-black/90 backdrop-blur-md border border-white/10 px-2 py-1 text-[10px] font-mono uppercase tracking-widest text-white shadow-2xl animate-in fade-in-0 zoom-in-95",
        className
      )}
    >
      {children}
